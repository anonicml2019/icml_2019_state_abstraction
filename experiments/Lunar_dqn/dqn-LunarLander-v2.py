...
for ep in range(num_episodes):
	...
	return_per_ep.append(total_reward)
	if ep>=10 and ep%10==0:
		print("Episode",ep)
		if np.mean(return_per_ep[-10:])>best_return_per_ep:
			print("saving file because there was an improvement from",best_return_per_ep,"to:",np.mean(return_per_ep[-10:]))
			saver.save(sess, 'models/my_dqn_model',global_step=ep,write_meta_graph=True)
			best_return_per_ep=np.mean(return_per_ep[-10:])
